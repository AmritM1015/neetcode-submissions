class Solution:
    def lastStoneWeight(self, stones: List[int]) -> int:
        # There is a bucket sort solution
        
        
        # We can use a Max Heap to keep track of the stones
        stones = [-s for s in stones] # we take the negative so that we can use Min Heap
        ...
        while len(stones) > 1:
            ...
            if second > first:
                heapq.heappush(stones, first-second) # We do not need to check whether the first is larger than the second because its sorted
        stones.append(0)
        return abs(stones[0])
